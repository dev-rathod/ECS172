...
class HiddenStateInjector:
    ...
    def install(self, llm: nn.Module, layer_idx: int):
        """Register the hook on the specified decoder layer."""
        # The attribute path to the decoder layers differs between a plain model
        # and one that's been wrapped by PEFT/LoRA.
        if hasattr(llm, "model") and hasattr(llm.model, "layers"):
            layer = llm.model.layers[layer_idx]
        elif hasattr(llm, "base_model"):
            layer = llm.base_model.model.layers[layer_idx]
        else:
...
def find_candidate_positions(
    ...
    for i in range(n_candidates):
        letter = LETTERS[i]
        # Look for the newline + letter + period that marks each candidate entry.
        marker = f"\n{letter}."
        char_idx = prompt.find(marker)
        if char_idx == -1:
            # The very first candidate might not have a leading newline, so try without.
            marker = f"{letter}."
            char_idx = prompt.find(marker)
...
def score_example(
    ...
    Args:
        inject_input: if True, also add adapter vectors at the input embedding
                      layer (layer 0). Same adapter.pt, dual injection.
        input_alpha: scaling factor for input-layer injection.
    """
    ...
    # If dual injection is on, add the adapter vectors directly into the input
    # embeddings before the first layer runs. This lets the signal propagate
    # through the full network rather than only from the target layer onward.
    if inject_input:
        embed_layer = llm.get_input_embeddings()
        input_embeds = embed_layer(inputs.input_ids).to(dtype=dtype)
        for pos, vec in zip(valid_pos, valid_vec):
            input_embeds[0, pos, :] += input_alpha * vec.to(dtype=dtype)
        fwd_kwargs = dict(inputs_embeds=input_embeds)
    else:
        fwd_kwargs = dict(input_ids=inputs.input_ids)

    # Register candidates with the hook, run the forward pass, then clear it.
    # The hook handles the mid-network injection at the target layer.
...
def parse_args():
    ...
    p.add_argument(
        ...
        help="Scaling factor for layer-8 injection: h += alpha * z",
    )
    p.add_argument("--inject-input", action="store_true",
                    help="Also add adapter vectors at input embedding layer (layer 0)")
    p.add_argument("--input-alpha", type=float, default=1.0,
                    help="Scaling factor for input-layer injection (only with --inject-input)")
...
def main():
    ...
    # Attach the injection hook to the target layer — this is what does the mid-network work.
    ...
    if args.inject_input:
        print(f"[input] also injecting at input embeddings, input_alpha={args.input_alpha}")
    ...
    for i, ex in enumerate(examples):
        ranked, probs = score_example(
            ...
            inject_input=args.inject_input,
        ...
        # Print a few predictions at the start so we can sanity-check things look right.
        if i < 3:
            pred = ranked[0]
            print(
    ...
    # Clean up the hook now that we're done with inference.
    injector.remove()

    # Print results. The condition label includes the alpha values so it's easy to
    # compare runs at a glance when you're sweeping hyperparameters.
    model_label = Path(args.model).name if "/" not in str(args.model) else args.model.split("/")[-1]
    if args.inject_input:
        condition = f"Dual L0+L{args.target_layer} a={args.input_alpha}/{args.alpha} ({model_label})"
    else:
