...
class InjectedDecoderLayer(nn.Module):
    ...
    def __init__(self, original_layer: nn.Module, n_prefix: int):
        super().__init__()
        self.layer = original_layer     # the real decoder layer
    ...
    def forward(self, hidden_states: torch.Tensor, **kwargs):
        ...
        # Pass-through when no prefix is active
        ...
        #   Prefix tokens get position 0 (position-free); real tokens
        #   keep their original positions so RoPE stays consistent
        ...
        #   Newer HF transformers pre-compute RoPE (cos, sin) and pass
        #   them as a tuple.  We replicate the position-0 embedding for
        #   the prefix tokens so the layer doesn't crash on shape mismatch.
        ...
        # ── 5. Forward through the real decoder layer ────────────
        outputs = self.layer(extended_hs, **kw)

        # ── 6. Strip prefix from output hidden states ────────────
        out_hs = outputs[0][:, n:, :]
...
def _extend_mask( mask: torch.Tensor, n_prefix: int, device: torch.device,) -> torch.Tensor:
    ...
    if mask.dim() == 4:
        ...
        ext[:, :, n:, n:] = mask            # bottom-right = original causal
        # everything else stays 0 → "allowed"
        return ext
